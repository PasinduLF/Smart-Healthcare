...
import { Calendar, Clock, Video, FileSignature, LogOut, User, Activity, X, Bell, Trash2 } from 'lucide-react';
...
export default function DoctorDashboard() {
    ...
    const [profileData, setProfileData] = useState({ name: '', specialization: '', experience: 0, availability: '', phone: '' });
    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            try {
                const apptRes = await axios.get(`http://localhost:3000/api/appointments/doctor/${doctorId}`, { headers: { Authorization: `Bearer ${token}` } });
                setAppointments(apptRes.data);

                // Resolve patient names
                const uniqueIds = [...new Set(apptRes.data.map(a => a.patientId))];
                const nameMap = {};
                await Promise.all(uniqueIds.map(async (pid) => {
                    try {
                        const p = await axios.get(`http://localhost:3000/api/patients/profile/${pid}`, { headers: { Authorization: `Bearer ${token}` } });
                        nameMap[pid] = p.data.name;
                    } catch { nameMap[pid] = pid; }
                }));
                setPatientNames(nameMap);
                setDoctorPatients(uniqueIds.map(pid => ({ id: pid, name: nameMap[pid] || pid })));
                
                const presRes = await axios.get(`http://localhost:3000/api/doctors/prescriptions/doctor/${doctorId}`, { headers: { Authorization: `Bearer ${token}` } });
                setPrescriptions(presRes.data);
                
                const profRes = await axios.get(`http://localhost:3000/api/doctors/profile/${doctorId}`, { headers: { Authorization: `Bearer ${token}` } });
                setProfileData({
                ...
                try {
                    const nRes = await axios.get(`http://localhost:3000/api/notifications/history/doctor/${doctorId}`);
    ...
    const handleEndConsultation = async () => {
        ...
        if (activeCallApptId) {
            try {
                await axios.put(`http://localhost:3000/api/appointments/complete/${activeCallApptId}`, {}, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setAppointments(appointments.map(a => a._id === activeCallApptId ? { ...a, status: 'completed' } : a));
            } catch (err) {
    ...
    const handleUpdateProfile = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:3000/api/doctors/profile/${doctorId}`, {
                ...profileData,
    ...
    const deleteNotification = async (notificationId) => {
        if (!doctorId) return;
        try {
            await axios.delete(`http://localhost:3000/api/notifications/history/doctor/${doctorId}/${notificationId}`);
    ...
    const clearNotifications = async () => {
        if (!doctorId) return;
        try {
            await axios.delete(`http://localhost:3000/api/notifications/history/doctor/${doctorId}`);
            setNotifications([]);
        } catch (err) {
            console.error(err);
                ...
                <Routes>
                    ...
                    <Route path="schedule" element={
                        <div className="space-y-6">
                            ...
                            <form onSubmit={handleUpdateProfile} className="space-y-4 max-w-xl">
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-gray-700">Phone Number (for SMS notifications)</label>
                                    <input type="text" className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" placeholder="+1 234 567 890" value={profileData.phone} onChange={e => setProfileData({...profileData, phone: e.target.value})} />
                                </div>
                    ...
                    <Route path="appointments" element={
                        <div className="relative">
                            ...
                            <div className="mb-4 rounded-xl border border-teal-100 bg-teal-50/60 px-4 py-3 text-sm text-teal-900">
                                <p className="font-medium">Logged in doctor account: Dr. {profileData.name || user?.name || 'Unknown'}</p>
                                <p className="text-teal-700">Appointments appear only for this exact doctor profile. If a patient booked another doctor account, it will not show here.</p>
                                {doctorId && <p className="text-xs text-teal-600 mt-1">Doctor ID: {doctorId}</p>}
                            </div>
                                        ...
                                        <div className="mt-4">
                                            ...
                                            <div className="flex gap-2 flex-wrap">
                                                {selectedPatient.reports.map((report, i) => (
                                                    <a key={i} href={report.url || report.fileUrl} target="_blank" rel="noreferrer" className="px-3 py-1 bg-white border rounded text-xs text-blue-600 hover:underline">
                                                        {report.originalName}
                                                    </a>
                                                ))}
                                            </div>
                            ...
                            <div className="space-y-4">
                                {appointments.length === 0 ? (
                                    <div className="rounded-xl border border-gray-200 bg-white/70 px-4 py-5 text-sm text-gray-600">
                                        <p className="font-medium text-gray-800">No appointments scheduled for this doctor account.</p>
                                        <p className="mt-1">If you just booked an appointment on the patient side, make sure you are logged into the same doctor profile that was selected during booking.</p>
                                    </div>
                                    ...
                                    <div key={appt._id} className={`p-4 border rounded-xl bg-white/50 flex flex-col md:flex-row justify-between md:items-center ${appt.status === 'cancelled' || appt.status === 'rejected' ? 'opacity-50' : ''}`}>
                                        <div className="mb-4 md:mb-0">
                                            <h3 className="font-bold cursor-pointer hover:text-teal-600" onClick={() => viewPatientProfile(appt.patientId)}>
                                                Patient: <span className="underline">{patientNames[appt.patientId] || appt.patientId}</span>
                                            </h3>
                    ...
                    <Route path="telemedicine" element={
                        <div>
                            ...
                            {activeCall ? (
                                <VideoCall channelName={activeCall} onEndCall={handleEndConsultation} />
                            ) : (
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            ...
                                            <select
                                                ...
                                                value={newScript.patientId}
                                                onChange={e => setNewScript({ ...newScript, patientId: e.target.value })}
                                            >
                                                <option value="">Choose a patient from your appointments</option>
                                                {doctorPatients.map(p => (
                                                    <option key={p.id} value={p.id}>{p.name} ({p.id.slice(-6)})</option>
                                                ))}
                                            </select>
                                            {doctorPatients.length === 0 && (
                                                <p className="text-xs text-gray-500">No appointment patients found yet. Accept/book an appointment first.</p>
                                            )}
                                        </div>
                                        <div className="space-y-2">
                                ...
                                {prescriptions.length === 0 ? <p className="text-gray-500">No prescriptions issued yet.</p> : prescriptions.map(script => (
                                    <div key={script._id} className="p-4 border rounded-xl bg-white flex flex-col md:flex-row justify-between md:items-center">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-400 mt-2">Patient: {patientNames[script.patientId] || script.patientId} • Issued: {new Date(script.issuedAt).toLocaleString()}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <Route path="notifications" element={
                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                <h2 className="text-xl font-semibold flex items-center gap-2"><Bell className="w-5 h-5 text-teal-600" /> Notification History</h2>
