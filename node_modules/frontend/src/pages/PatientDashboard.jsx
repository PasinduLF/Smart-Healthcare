...
import { Search, Calendar, Video, FileText, Brain, LogOut, Activity, User, Pill, Bell, Trash2, Copy } from 'lucide-react';
...
export default function PatientDashboard() {
    ...
    const getDoctorName = (id) => doctorNameMap[id] || doctors.find(d => d._id === id)?.name || id;
    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            try {
                ...
                // Resolve doctor names for prescriptions so patient can track who prescribed what
                const uniqueDoctorIds = [...new Set((presRes.data || []).map(s => s.doctorId).filter(Boolean))];
                const names = {};
                await Promise.all(uniqueDoctorIds.map(async (did) => {
                    try {
                        const d = await axios.get(`http://localhost:3000/api/doctors/profile/${did}`, { headers: { Authorization: `Bearer ${token}` } });
                        names[did] = d.data?.name || did;
                    } catch {
                        names[did] = doctors.find(doc => doc._id === did)?.name || did;
                    }
                }));
                setDoctorNameMap(names);

                // Fetch notification history
                try {
                    const nRes = await axios.get(`http://localhost:3000/api/notifications/history/patient/${user.id}`);
    ...
    const handleBookAppointment = (doctorId) => {
        ...
        setBookingFor(doctorId);
        setBookingForm({ date: '', time: '' });
    };

    const handleConfirmBooking = async (doctorId) => {
        if (!bookingForm.date || !bookingForm.time) return alert('Please select a date and time');
        const selectedDoctor = doctors.find(d => d._id === doctorId);
        const doctorName = selectedDoctor?.name || 'this doctor';
        const confirmed = window.confirm(`Confirm appointment with Dr. ${doctorName} on ${bookingForm.date} at ${bookingForm.time}?`);
        if (!confirmed) return;
        try {
            ...
            alert(`Appointment booked with Dr. ${doctorName}. You will receive a confirmation email/SMS.`);
            setBookingFor(null);
        } catch (err) {
            console.error(err);
            if (err.response?.status === 401 || err.response?.status === 403) {
                alert('Your session expired. Please login again and retry booking.');
                logout();
                navigate('/');
                return;
            }
            const apiError = err.response?.data?.error;
            alert(apiError ? `Failed to book appointment: ${apiError}` : 'Failed to book appointment. Please try again.');
        }
    };

    const handlePayment = async (appt) => {
        try {
            const doctorName = doctors.find(d => d._id === appt.doctorId)?.name || 'Doctor';
            const res = await axios.post('http://localhost:3000/api/payments/create-checkout-session', {
                amount: 5000,
                currency: 'usd',
                description: `Consultation with Dr. ${doctorName} on ${appt.date}`
            }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const deleteNotification = async (notificationId) => {
        try {
            await axios.delete(`http://localhost:3000/api/notifications/history/patient/${user.id}/${notificationId}`);
    ...
    const clearNotifications = async () => {
        try {
            await axios.delete(`http://localhost:3000/api/notifications/history/patient/${user.id}`);
            setNotifications([]);
        } catch (err) {
            console.error(err);
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex gap-4">
                                <input
                                    ...
                                    className="flex-1 px-4 py-3 rounded-lg border focus:ring-2 focus:ring-blue-500 outline-none"
                                    value={searchQuery}
                                    onChange={e => setSearchQuery(e.target.value)}
                                    onKeyDown={e => {
                                        if (e.key === 'Enter') {
                                            const q = searchQuery.trim().toLowerCase();
                                            setFilteredDoctors(q ? doctors.filter(d => d.name?.toLowerCase().includes(q) || d.specialty?.toLowerCase().includes(q)) : doctors);
                                        }
                                    }}
                                />
                                <button
                            ...
                            <div className="mt-8 grid md:grid-cols-2 gap-4">
                                {filteredDoctors.length === 0 ? <p className="text-gray-500">No doctors found.</p> : filteredDoctors.map(doc => (
                                    <div key={doc._id} className="p-4 border rounded-xl hover:shadow-md transition bg-white/50">
                                        <h3 className="font-bold text-lg">{doc.name}</h3>
                                        <p className="text-gray-500 text-sm">{doc.specialty} • Max Patients: {doc.maxPatients}</p>
                                        {doc.availability && <p className="text-xs text-teal-600 mt-1">🕐 {doc.availability}</p>}
                                        {bookingFor === doc._id ? (
                                            <div className="mt-3 space-y-2">
                                                <input type="date" min={new Date().toISOString().split('T')[0]} className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none" value={bookingForm.date} onChange={e => setBookingForm({...bookingForm, date: e.target.value})} />
                                                <input type="time" className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none" value={bookingForm.time} onChange={e => setBookingForm({...bookingForm, time: e.target.value})} />
                                                <div className="flex gap-2">
                                                    <button onClick={() => handleConfirmBooking(doc._id)} className="flex-1 py-2 bg-teal-600 text-white text-sm font-medium rounded-lg hover:bg-teal-700 transition">Confirm Booking</button>
                                                    <button onClick={() => setBookingFor(null)} className="px-4 py-2 bg-gray-100 text-gray-600 text-sm rounded-lg hover:bg-gray-200 transition">Cancel</button>
                                                </div>
                                            </div>
                                        ) : (
                                            <button 
                                                onClick={() => handleBookAppointment(doc._id)}
                                                className="w-full mt-3 py-2 bg-teal-50 text-teal-700 font-medium rounded-lg hover:bg-teal-100 transition">
                                                Book Appointment
                                            </button>
                                        )}
                                    </div>
                                ...
                                {appointments.length === 0 ? <p className="text-gray-500">No appointments scheduled.</p> : appointments.map(appt => (
                                    <div key={appt._id} className={`p-6 border rounded-xl bg-white/50 flex flex-col md:flex-row justify-between items-start md:items-center ${appt.status === 'cancelled' ? 'opacity-50' : ''}`}>
                                        <div className={`mb-4 md:mb-0 ${appt.status === 'cancelled' || appt.status === 'rejected' ? 'opacity-50' : ''}`}>
                                            <h3 className="font-bold">Dr. {getDoctorName(appt.doctorId)}</h3>
                                        ...
                                        <div className="flex gap-2">
                                            {appt.status !== 'cancelled' && appt.status !== 'rejected' && rescheduleData.id !== appt._id && (
                                                <>
                                                    ...
                                                    <button onClick={() => handlePayment(appt)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm font-medium">Pay Now</button>
                                                </>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <Route path="profile" element={
                        <div className="space-y-6">
                            ...
                            <div className="p-4 border rounded-xl bg-blue-50/50 flex items-center justify-between">
                                <div>
                                    <p className="text-xs text-gray-500">Patient ID (for prescriptions/reference)</p>
                                    <p className="font-mono text-sm text-gray-800 break-all">{user?.id || 'N/A'}</p>
                                </div>
                                <button onClick={copyPatientId} className="px-3 py-2 text-sm rounded-lg bg-white border hover:bg-gray-50 flex items-center gap-2">
                                ...
                                {myPrescriptions.length === 0 ? <p className="text-gray-500">No prescriptions found.</p> : myPrescriptions.map(script => (
                                    <div key={script._id} className="p-4 border rounded-xl bg-white flex flex-col md:flex-row justify-between md:items-center">
                                        <div>
                                            ...
                                            <p className="text-xs text-blue-500 font-medium mt-2">Prescribed By Dr. {getDoctorName(script.doctorId)} • Issued: {new Date(script.issuedAt).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <Route path="notifications" element={
                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                <h2 className="text-xl font-semibold flex items-center gap-2"><Bell className="w-5 h-5 text-blue-600" /> Notification History</h2>
                                {notifications.length > 0 && (
                                    <button onClick={clearNotifications} className="px-3 py-2 text-sm rounded-lg bg-red-50 text-red-600 hover:bg-red-100">Clear All</button>
                                )}
                            </div>
                            {notifications.length === 0 ? <p className="text-gray-500">No notifications yet.</p> : notifications.map(n => (
                                <div key={n._id} className="p-4 border rounded-xl bg-white/50 flex items-start justify-between gap-4">
                                    <div className={`w-2 h-2 mt-2 rounded-full flex-shrink-0 ${n.type === 'booking' ? 'bg-blue-500' : n.type === 'status' ? 'bg-amber-500' : 'bg-green-500'}`}></div>
                                    ...
                                    <button onClick={() => deleteNotification(n._id)} className="p-2 rounded-lg text-red-500 hover:bg-red-50" title="Delete notification">
